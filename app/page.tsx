...
import BacRangeCard from '@/components/BacRangeCard'
...
export default async function Home() {
  ...
  const [{data:entries}, {data:readings}, {data:reminders}] = await Promise.all([
    ...
    supabase.from('bac_readings').select('bac,measured_at').gte('measured_at',start.toISOString()).lt('measured_at',end.toISOString()).order('measured_at',{ascending:false}),
  ...
  return <AppShell>
    ...
    <div className="card metric"><span className="sub">Alcohol</span><b>{drinks.toFixed(drinks%1?1:0)}</b><span className="tiny">standard drinks</span></div>
    <BacRangeCard readings={readings||[]} />
